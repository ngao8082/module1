...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    if a < b and b < c:
        assert a < c, f"Transitive property: {a} < {b} and {b} < {c}, but not {a} < {c}"
    # TODO: Implement for Task 0.2.
    raise NotImplementedError("Need to implement for Task 0.2")
...
@pytest.mark.task0_2
@given(small_floats, small_floats, small_floats)
def test_other(xx:float, yy:float, zz:float) -> None:
    """
    Write a test that ensures some other property holds for your functions.
    """
    test_sigmoid_results=test_sigmoid(xx)
    test_transitive_results=test_transitive(xx,yy,zz)
    test_distribute_result=test_distribute(xx,yy,zz)
    test_symmetric_result=test_symmetric(xx,yy)
    assert_close(test_transitive_results,test_transitive(xx,yy,zz))
    assert_close(test_distribute_result,test_distribute(xx,yy,zz))
    assert_close(test_symmetric_result,test_symmetric(xx,yy))
    assert_close(test_sigmoid_results, test_sigmoid(xx))
    # TODO: Implement for Task 0.2.
    raise NotImplementedError("Need to implement for Task 0.2")
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    sum2 = sum([x + y for x in ls1 for y in ls2])
    assert math.isclose(sum1, sum2, rel_tol=1e-9)
